...
bilateral_sex_interaction <- function(dependent, independent, sex_variable, covariates, dataset, hemisphere_col, id_col, structural_subtype){
  ...
  for (i in dependent){
    ...
    output[i,"estimate"] <- summary(lme1)[["coefficients"]][index, "Estimate"]
    output[i,"SE"] <- summary(lme1)[["coefficients"]][index, "Std. Error"]
    output[i,"p"] <- summary(lme1)[["coefficients"]][index, "Pr(>|t|)"]
  }
...
unilateral_sex_interaction <- function(dependent, independent, sex_variable, covariates, dataset, structural_subtype){
  ...
  for (i in dependent){
    ...
    output[i,"brain_phenotype"] <- i
    output[i,"disorder"] <- independent
    output[i, "estimate"] <- summary(glm1)[["coefficients"]][index, "Estimate"]
    output[i, "SE"] <- summary(glm1)[["coefficients"]][index, "Std. Error"]
    output[i, "p"] <- summary(glm1)[["coefficients"]][index, "Pr(>|t|)"]
...
bilateral_sex_interaction_group <- function(dependent, independent, sex_variable, covariates, dataset, hemisphere_col, id_col, structural_subtype){
  ...
  for (i in dependent){
    ...
    if (anova1[paste0(independent, ":", sex_variable), "Pr(>F)"] < 0.05){
      ...
      output[output$brain_phenotype == i, "estimate"] <- as.data.frame(contrast1)$estimate
      output[output$brain_phenotype == i, "SE"] <- as.data.frame(contrast1)$SE
      output[output$brain_phenotype == i, "p"] <- as.data.frame(contrast1)$p.value
...
unilateral_sex_interaction_group <- function(dependent, independent, sex_variable, covariates, datasets, structural_subtype){
...
for (sex in c("females", "males")){
  for (disorder in c("chronic_pain", "recurrent_depression")){
   ...
   df_sex_interactions <- filter(CP_MDD_structure_sex_interaction_all, disorder == disorder)
