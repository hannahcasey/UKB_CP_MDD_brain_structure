...
UKB_CP <- read.csv("/../UKB_chronic_pain_phenotype/output/UKB_chronic_pain_phenotype_imaging.csv")
...
depression_mean_full <- round(mean(UKB_CP_MDD_imaging_covariates$current_depression, na.rm = T),2)
depression_mean_female <- round(mean(UKB_CP_MDD_imaging_covariates_female$current_depression, na.rm = T),2)
depression_mean_male <- round(mean(UKB_CP_MDD_imaging_covariates_male$current_depression, na.rm = T),2)
depression_sd_full <- round(sd(UKB_CP_MDD_imaging_covariates$current_depression, na.rm = T),2)
depression_sd_female <- round(sd(UKB_CP_MDD_imaging_covariates_female$current_depression, na.rm = T),2)
depression_sd_male <- round(sd(UKB_CP_MDD_imaging_covariates_male$current_depression, na.rm = T),2)

chronic_pain_percent_full <- round(sum(UKB_CP_MDD_imaging_covariates$chronic_pain_status == 1, na.rm = T)/n_full * 100,2)
chronic_pain_percent_female <- round(sum(UKB_CP_MDD_imaging_covariates_female$chronic_pain_status == 1, na.rm = T)/n_female * 100,2)
chronic_pain_percent_male <- round(sum(UKB_CP_MDD_imaging_covariates_male$chronic_pain_status == 1, na.rm = T)/n_male * 100,2)


## Depression in those with chronic pain
CP_depression_mean_full <- round(mean(subset(UKB_CP_MDD_imaging_covariates, chronic_pain_status == 1)$current_depression, na.rm = T),2)
CP_depression_mean_female <- round(mean(subset(UKB_CP_MDD_imaging_covariates_female, chronic_pain_status == 1)$current_depression, na.rm = T),2)
CP_depression_mean_male <- round(mean(subset(UKB_CP_MDD_imaging_covariates_male, chronic_pain_status == 1)$current_depression, na.rm = T),2)
CP_depression_sd_full <- round(sd(subset(UKB_CP_MDD_imaging_covariates, chronic_pain_status == 1)$current_depression, na.rm = T),2)
CP_depression_sd_female <- round(sd(subset(UKB_CP_MDD_imaging_covariates_female, chronic_pain_status == 1)$current_depression, na.rm = T),2)
CP_depression_sd_male <- round(sd(subset(UKB_CP_MDD_imaging_covariates_male, chronic_pain_status == 1)$current_depression, na.rm = T),2)


descriptives_df <- data.frame(
  Variables = c("Mean Age (SD)", "Chronic Pain (%)", "Mean Depression Score (SD)", "Mean Depression Score in Chronic Pain (SD)"), 
  full_temp = c(
    paste0(age_mean_full, " (", age_sd_full, ")"), ## Age full sample
    paste0(chronic_pain_percent_full, "%"), ## Chronic pain full sample
    paste0(depression_mean_full, " (", depression_sd_full, ")"), ## Depression full sample
    paste0(CP_depression_mean_full, " (", CP_depression_sd_full, ")") ## Depression in chronic pain full sample
    ),
  female_temp = c(
    paste0(age_mean_female, " (", age_sd_female, ")"), ## Age female sample
    paste0(chronic_pain_percent_female, "%"), ## Chronic pain female sample
    paste0(depression_mean_female, " (", depression_sd_female, ")"), ## Depression female sample
    paste0(CP_depression_mean_female, " (", CP_depression_sd_female, ")") ## Depression in chronic pain female sample
    ),
  male_temp = c(
    paste0(age_mean_male, " (", age_sd_male, ")"), ## Age male sample
    paste0(chronic_pain_percent_male, "%"), ## Chronic pain male sample
    paste0(depression_mean_male, " (", depression_sd_male, ")"), ## Depression male sample
    paste0(CP_depression_mean_male, " (", CP_depression_sd_male, ")") ## Depression in chronic pain male sample
    ))
...
brain_structure_covariates_T1 <- c("sex", "age", "age_squared", "ICV", "assessment_centre_first_imaging")
brain_structure_covariates_diffusion <- c("sex", "age", "age_squared", "assessment_centre_first_imaging")
...
for (disorder in c("chronic_pain_status", "current_depression")) {
