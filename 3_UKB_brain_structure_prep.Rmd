...
UKB_imaging_covariates <- UKB_imaging_covariates %>%
  mutate(assessment_centre_first_imaging = recode(assessment_centre_first_imaging,
    `11027` = "Newcastle",
    `11025` = "Cheadle",
    `11026` = "Reading",
    `11028` = "Bristol"
...
PC_res_MD_association_fibres_PCA <- princomp(UKB_imaging_covariates_temp[,MD_association_fibres])
temp <- PC_res_MD_association_fibres_PCA$sdev^2 / sum(PC_res_MD_association_fibres_PCA$sdev^2)
PC_res_MD_association_fibres_VE <- temp[1]
                                             
PC_res_MD_thalamic_radiations <- princomp(UKB_imaging_covariates_temp[,MD_thalamic_radiations],scores = TRUE)$scores[,1]

PC_res_MD_thalamic_radiations_PCA <- princomp(UKB_imaging_covariates_temp[,MD_thalamic_radiations])
temp <- PC_res_MD_thalamic_radiations_PCA$sdev^2 / sum(PC_res_MD_thalamic_radiations_PCA$sdev^2)
PC_res_MD_thalamic_radiations_VE <- temp[1]

PC_res_MD_projection_fibres <- princomp(UKB_imaging_covariates_temp[,MD_projection_fibres],scores = TRUE)$scores[,1]

PC_res_MD_projection_fibres_PCA <- princomp(UKB_imaging_covariates_temp[,MD_projection_fibres])
temp <- PC_res_MD_projection_fibres_PCA$sdev^2 / sum(PC_res_MD_projection_fibres_PCA$sdev^2)
PC_res_MD_projection_fibres_VE <- temp[1]

PC_res_MD_global <- princomp(UKB_imaging_covariates_temp[,MD_global],scores = TRUE)$scores[,1]

PC_res_MD_global_PCA <- princomp(UKB_imaging_covariates_temp[,MD_global])
temp <- PC_res_MD_global_PCA$sdev^2 / sum(PC_res_MD_global_PCA$sdev^2)
PC_res_MD_global_VE <- temp[1]
...
PC_res_FA_association_fibres_PCA <- princomp(UKB_imaging_covariates_temp[,FA_association_fibres])
temp <- PC_res_FA_association_fibres_PCA$sdev^2 / sum(PC_res_FA_association_fibres_PCA$sdev^2)
PC_res_FA_association_fibres_VE <- temp[1]
                                             
PC_res_FA_thalamic_radiations <- princomp(UKB_imaging_covariates_temp[,FA_thalamic_radiations],scores = TRUE)$scores[,1]

PC_res_FA_thalamic_radiations_PCA <- princomp(UKB_imaging_covariates_temp[,FA_thalamic_radiations])
temp <- PC_res_FA_thalamic_radiations_PCA$sdev^2 / sum(PC_res_FA_thalamic_radiations_PCA$sdev^2)
PC_res_FA_thalamic_radiations_VE <- temp[1]

PC_res_FA_projection_fibres <- princomp(UKB_imaging_covariates_temp[,FA_projection_fibres],scores = TRUE)$scores[,1]

PC_res_FA_projection_fibres_PCA <- princomp(UKB_imaging_covariates_temp[,FA_projection_fibres])
temp <- PC_res_FA_projection_fibres_PCA$sdev^2 / sum(PC_res_FA_projection_fibres_PCA$sdev^2)
PC_res_FA_projection_fibres_VE <- temp[1]

PC_res_FA_global <- princomp(UKB_imaging_covariates_temp[,FA_global],scores = TRUE)$scores[,1]

PC_res_FA_global_PCA <- princomp(UKB_imaging_covariates_temp[,FA_global])
temp <- PC_res_FA_global_PCA$sdev^2 / sum(PC_res_FA_global_PCA$sdev^2)
PC_res_FA_global_VE <- temp[1]
...
DTI_VE_df <- data.frame(
  "DTI_measure" = c("MD global", "MD association fibres", "MD projection fibres", "MD thalamic associations", "FA global", "FA association fibres", "FA projection fibres", "FA thalamic associations"),
  "variance_explained" = c(PC_res_MD_global_VE, PC_res_MD_association_fibres_VE, PC_res_MD_projection_fibres_VE, PC_res_MD_thalamic_radiations_VE, PC_res_FA_global_VE, PC_res_FA_association_fibres_VE, PC_res_FA_projection_fibres_VE, PC_res_FA_thalamic_radiations_VE)
)

write.csv(DTI_VE_df, "~/Desktop/PhD/projects/UKB_CP_MDD_brain_structure/output/DTI_PCA_variance_explained.csv", row.names = F)
...
## Remove outliers - remove any imaging structre value with SD <3 from mean

```{r Remove outliers}
DTI_brain_structure_names <- grep("FA|MD", names(UKB_imaging_covariates), value = TRUE)
T1_brain_structure_names <- grep("volume|area|thickness", names(UKB_imaging_covariates), value = TRUE)

## Get a dataframe only containing brain structures for outlier removal - will be used to calculate zscores
DTI_brain_structure_df <- UKB_imaging_covariates[,DTI_brain_structure_names]
T1_brain_structure_df <- UKB_imaging_covariates[,T1_brain_structure_names]

## Calculate Z score for each imaging measure
DTI_z_scores <- as.data.frame(sapply(DTI_brain_structure_df, function(DTI_brain_structure_df) (abs(DTI_brain_structure_df-mean(DTI_brain_structure_df, na.rm = TRUE))/sd(DTI_brain_structure_df, na.rm = TRUE))))    

T1_z_scores <- as.data.frame(sapply(T1_brain_structure_df, function(T1_brain_structure_df) (abs(T1_brain_structure_df-mean(T1_brain_structure_df, na.rm = TRUE))/sd(T1_brain_structure_df, na.rm = TRUE))))    

## Get ID of those with at least one brain structure > 5SD from mean
DTI_outlier_ids <- unique(UKB_imaging_covariates$f.eid[rowSums(DTI_z_scores>5, na.rm = T) > 0])
T1_outlier_ids <- unique(UKB_imaging_covariates$f.eid[rowSums(T1_z_scores>5, na.rm = T) > 0])

outlier_ids <- unique(c(DTI_outlier_ids, T1_outlier_ids))

## Save list of outliers
write.csv(as.data.frame(DTI_outlier_ids), "~/Desktop/PhD/projects/UKB_CP_MDD_brain_structure/resources/DTI_imaging_outliers.csv", quote = F)
write.csv(as.data.frame(T1_outlier_ids), "~/Desktop/PhD/projects/UKB_CP_MDD_brain_structure/resources/T1_imaging_outliers.csv", quote = F)
write.csv(as.data.frame(outlier_ids), "~/Desktop/PhD/projects/UKB_CP_MDD_brain_structure/resources/imaging_outliers.csv", quote = F)
```
## Scale all brain structures and continuous covariates

```{r Scale data }
## Scale brain strucutre data
UKB_imaging_covariates$age_raw <- UKB_imaging_covariates$age
UKB_imaging_covariates$BMI_raw <- UKB_imaging_covariates$BMI
...
write.csv(UKB_imaging_covariates, "~/Desktop/PhD/projects/UKB_CP_MDD_brain_structure/resources/UKB_imaging_covariates_qc.csv", quote = F)
write.csv(UKB_imaging_covariates_long, "~/Desktop/PhD/projects/UKB_CP_MDD_brain_structure/resources/UKB_imaging_covariates_long_qc.csv", quote = F)
```
